package com.frontanilla.alejandria.modules.shaderplus;
...
public class Illumination2D implements ApplicationListener {
    ...
    private Texture texture, texture_n;

    private boolean flipY;
    private Texture normalBase;
    private OrthographicCamera cam;
    private SpriteBatch fxBatch, batch;
    ...
    private final Vector3 DEFAULT_LIGHT_POS = new Vector3(0f, 0f, 0.07f);
    // the color of our light
    private final Vector3 DEFAULT_LIGHT_COLOR = new Vector3(1f, 0.7f, 0.6f);
    // the ambient color (color to use when unlit)
    private final Vector3 DEFAULT_AMBIENT_COLOR = new Vector3(0.3f, 0.3f, 1f);
    // the attenuation factor: x=constant, y=linear, z=quadratic
    private final Vector3 DEFAULT_ATTENUATION = new Vector3(0.4f, 3f, 20f);
    // the ambient intensity (brightness to use when unlit)
    private final float DEFAULT_AMBIENT_INTENSITY = 0.2f;
    private final float DEFAULT_STRENGTH = 1f;

    private final Color NORMAL_VCOLOR = new Color(1f, 1f, 1f, DEFAULT_STRENGTH);

    // the position of our light in 3D space
    private Vector3 lightPos = new Vector3(DEFAULT_LIGHT_POS);
    // the resolution of our game/graphics
    private Vector2 resolution = new Vector2();
    // the current attenuation
    private Vector3 attenuation = new Vector3(DEFAULT_ATTENUATION);
    // the current ambient intensity
    private float ambientIntensity = DEFAULT_AMBIENT_INTENSITY;
    ...
    private DecimalFormat DEC_FMT = new DecimalFormat("0.00000");
    ...
    private final String TEXT = "Use number keys to adjust parameters:\n" +
